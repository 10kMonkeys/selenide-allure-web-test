...
public class ActionHelper {
    ...
    public static void dragAndDropRealistic(SelenideElement source, SelenideElement target) {
        actions().moveToElement(source)
                .clickAndHold()
                .pause(Duration.ofMillis(100))
                // Перемещаем к целевым координатам с небольшим смещением
                .moveByOffset(target.getLocation().getX() - source.getLocation().getX() + 10,
                        target.getLocation().getY() - source.getLocation().getY() + 10)
    ...
    public static void seleniumDoubleTap(SelenideElement element) {
